...
interface ActiveDraftProps {
  isLoading?: boolean;
  draft?: {
    id: string; // 👈 CRITICAL: We need the ID to route to the editor!
    novelTitle?: string;
    title: string;
    status: string;
    timeAgo: string;
...
export function ActiveDraft({ isLoading, draft }: ActiveDraftProps) {
  ...
  // 2. Premium Empty State (If no draft exists)
  if (!draft) {
    return (
      <section className="relative flex flex-col items-center justify-center gap-3 py-16 md:py-20 mb-6 md:mb-8 shrink-0 w-full group p-4 md:p-6 rounded-2xl border border-transparent md:hover:border-border/40 md:hover:bg-secondary/10 transition-all duration-500 cursor-pointer">
        ...
        <Link
          href="/project/new"
          className="mt-4 md:mt-6 px-6 py-2.5 text-[10px] md:text-xs font-bold uppercase tracking-widest text-foreground border border-border/50 rounded-full group-hover:bg-foreground group-hover:text-background transition-all duration-300"
        >
          Create New Novel
        </Link>
      </section>
  ...
  // 3. Loaded Data State (Clickable Link to the Editor)
  return (
    // 👇 Notice we wrapped the entire section in a Next.js Link
    <Link href={`/project/${draft.id}/write`} className="block w-full outline-none">
