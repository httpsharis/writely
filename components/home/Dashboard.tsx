  ...
  return (
    <div className="min-h-dvh bg-grid p-4 md:p-6 lg:px-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        >
          <Plus size={14} /> {creating ? 'Creating...' : 'New Novel'}
        </button>
      </div>

      {/* Grid */}
      {loading ? (
        <div className="flex min-h-50 items-center justify-center"><Spinner /></div>
      ) : novels.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-[repeat(auto-fill,minmax(300px,1fr))]">
          {novels.map((novel) => (
            ...
            >
              <div className="mb-2 font-mono text-[10px] uppercase tracking-wider opacity-50">{novel.status}</div>
              <div className="mb-2 text-xl font-extrabold uppercase">{novel.title}</div>
              {novel.description && <div className="mb-3 text-[13px] leading-relaxed text-gray-500">{novel.description}</div>}
