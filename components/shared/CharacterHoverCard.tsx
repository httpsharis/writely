  ...
  return (
    <>
      ...
      {hoverState && typeof document !== "undefined" && createPortal(
        ...
        >
          <div className="p-3.5 bg-card/95 backdrop-blur-xl border border-border rounded-xl shadow-2xl flex items-start gap-4 w-[360px]">
            <div className="w-12 h-12 rounded-full bg-secondary/50 flex-shrink-0 flex items-center justify-center overflow-hidden border border-border">
              {hoverState.char.avatarUrl ? (
                <Image src={getAvatarUrl(hoverState.char.avatarUrl, 96)} alt={hoverState.char.name} width={48} height={48} className="w-full h-full object-cover" />
              ) : (
                <span className="text-muted-foreground text-lg font-bold uppercase">{hoverState.char.name.charAt(0)}</span>
              )}
            </div>
            <div className="flex-1 min-w-0">
              <h4 className="text-foreground font-serif font-bold text-base mb-1 truncate">{hoverState.char.name}</h4>
      ...
      {isProfileOpen && activeCharacter && (
        <>
          ...
          />
          <div className="fixed bottom-0 left-0 right-0 md:bottom-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 z-[101] bg-card border border-border rounded-t-3xl md:rounded-3xl p-6 md:p-8 animate-in slide-in-from-bottom-full md:zoom-in-95 duration-300 shadow-2xl md:max-w-md w-full">
            <button 
              onClick={() => setIsProfileOpen(false)}
              className="absolute top-4 right-4 md:top-5 md:right-5 w-8 h-8 flex items-center justify-center rounded-full bg-secondary/60 text-muted-foreground hover:text-foreground cursor-pointer transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            
            <div className="flex flex-col items-center text-center mt-2">
              <div className="w-20 h-20 rounded-full bg-secondary/50 flex items-center justify-center overflow-hidden border-2 border-border mb-4 shadow-xl">
                {activeCharacter.avatarUrl ? (
                  <Image src={getAvatarUrl(activeCharacter.avatarUrl, 160)} alt={activeCharacter.name} width={80} height={80} className="w-full h-full object-cover" />
                ) : (
                  <span className="text-muted-foreground text-3xl font-bold uppercase">{activeCharacter.name.charAt(0)}</span>
                )}
              </div>
              <h4 className="text-foreground font-serif font-bold text-2xl mb-2">{activeCharacter.name}</h4>
              <p className="text-muted-foreground text-sm leading-relaxed max-w-sm whitespace-pre-wrap text-left w-full mt-4 bg-secondary/20 p-4 rounded-xl border border-border">
