    ...
    <section className="relative mb-16 md:mb-20">
      <div className="h-48 md:h-64 w-full rounded-3xl md:rounded-4xl bg-gradient-to-r from-primary/20 via-purple-500/20 to-emerald-500/20 border border-border relative overflow-hidden group">
        {myProfile?.coverImageUrl ? (
          <Image src={myProfile.coverImageUrl} alt="Cover" fill className="object-cover opacity-80" unoptimized />
        ) : (
        ...
        <button 
          onClick={handleShare}
          className="absolute top-4 right-4 md:top-6 md:right-6 flex items-center gap-2 px-4 py-2 bg-black/40 hover:bg-black/60 border border-white/10 rounded-full text-white backdrop-blur-md transition-all z-10"
        >
